...
6. TS 可以在代码执行前就完成代码的检查，减小了运行时异常的出现的几率；
7. TS 代码可以编译为任意版本的 JS 代码，可有效解决不同 JS 运行环境的兼容问题；
8. 同样的功能，TS 的代码量要大于 JS，但由于 TS 的代码结构更加清晰，变量类型更加明确，在后期代码的维护中 TS 却远远胜于 JS。
...
- 类型：
  ...
  |   any   |        \*         |             任意类型 （可以赋值给其他变量）             |
  | unknown |        \*         | 类型安全的 any （不能赋值给其他变量，需要使用类型断言） |
...
- unknown
  ...
  - ```typescript
    ...
    notSure = 'hello';
    // 类型断言
    let s: string = notSure as string;
    s = <string>notSure;
    ```
...
- 经过一系列的配置，使得 TS 和 webpack 已经结合到了一起，除了 webpack，开发中还经常需要结合 babel 来对代码进行转换以使其可以兼容到更多的浏览器，在上述步骤的基础上，通过以下步骤再将 babel 引入到项目中。
  ...
  1. 安装依赖包：
     ...
     - 共安装了 4 个包，分别是：
       ...
       - core-js
         - core-js 用来使老版本的浏览器支持新版 ES 的 API
